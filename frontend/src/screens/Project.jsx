...
const Project = () => {
    ...
    function addCollaborators() {
        ...
        const targetProjectId = project._id || location.state?.project?._id;
        ...
        axios.put("/projects/add-user", {
            ...
            setSelectedUserId(new Set()) // Reset selections
            if (res.data.project) {
                setProject(res.data.project)
            }
        }).catch(err => {
            console.error("Failed to add collaborators:", err.response?.data || err.message)
        })
    }
    ...
    function WriteAiMessage(rawMessage) {
        ...
        try {
            ...
            // Fallback if message is plaintext
        }
    ...
    function saveFileTree(ft) {
        axios.put('/projects/update-file-tree', {
            ...
            console.log("File tree updated:", res.data)
        }).catch(err => {
                ...
                <div className={`sidePanel w-full h-full flex flex-col gap-2 bg-slate-50 absolute transition-all ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'} top-0 z-20`}>
                    ...
                    <div className="users flex flex-col gap-2 overflow-auto">
                        {project.users && project.users.map(u => {
                            ...
                            return (
                                <div key={uId} className="user cursor-pointer hover:bg-slate-200 p-2 flex gap-2 items-center">
                    ...
                    <div className="top flex justify-between w-full bg-slate-200">
                        ...
                        <div className="actions flex gap-2">
                            <button
                                ...
                                className='p-2 px-4 bg-slate-800 text-white hover:bg-slate-900'
                            >
                                Run
                            </button>
                        </div>
                    ...
                    <div className="flex min-w-96 flex-col h-full border-l border-slate-300">
                        <div className="address-bar">
                            <input 
                                ...
                                value={iframeUrl} 
                                className="w-full p-2 px-4 bg-slate-200 outline-none" 
                            />
                        </div>
                    ...
                    <div className="bg-white p-4 rounded-md w-96 max-w-full relative shadow-lg">
                        ...
                        <div className="users-list flex flex-col gap-2 mb-16 max-h-96 overflow-auto">
                            {users.map(u => {
                                ...
                                const isSelected = selectedUserId.has(userId);

                                return (
                                    <div 
                                        key={userId} 
                                        className={`user cursor-pointer hover:bg-slate-200 p-2 flex gap-2 items-center rounded-md ${isSelected ? 'bg-slate-200 border-l-4 border-blue-600' : ''}`} 
